...
const WardrobeHeader = ({ itemCount, savedOutfits, aiSuggestions, onAddClick }: WardrobeHeaderProps) => {
  const stats = [
    { icon: Shirt, value: itemCount, label: "Món đồ", accent: "bg-accent/10 text-accent" },
    { icon: Heart, value: savedOutfits, label: "Outfit đã lưu", accent: "bg-teal-light text-teal" },
    { icon: Sparkles, value: aiSuggestions, label: "Gợi ý từ AI", accent: "bg-accent/10 text-accent" },
  ];
    ...
    <div className="px-6 pt-24 pb-8">
      <div className="container mx-auto max-w-6xl">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.4 }}
          ...
          {stats.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.25 + i * 0.08 }}
