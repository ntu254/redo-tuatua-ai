        ...
        <div className="hidden md:flex flex-1 flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-6">
            ...
            {sampleOutfits.map((outfit) =>
              ...
              className="bg-background border border-border overflow-hidden hover:shadow-md transition-shadow">
                {/* Horizontal layout: image left, products right */}
                <div className="flex">
                  {/* Left: outfit image */}
                  <div className="relative w-[200px] shrink-0">
                    ...
                    className={`absolute top-2 right-2 p-1.5 rounded-md bg-background/80 backdrop-blur-sm transition-colors ${saved.has(outfit.id) ? "text-accent" : "text-muted-foreground"}`}>
                      <Bookmark className="w-3.5 h-3.5" fill={saved.has(outfit.id) ? "currentColor" : "none"} />
                    </button>
                    <div className="absolute bottom-2 left-2">
                      <span className="bg-foreground/80 text-background text-[9px] font-body font-medium px-2 py-1 rounded backdrop-blur-sm">
                        {outfit.style}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 p-4 flex flex-col">
                    ...
                    {/* AI comment */}
                    <div className="bg-accent/5 border border-accent/10 rounded-md px-3 py-2 mb-3">
                    ...
                    <div className="space-y-2 flex-1">
                      ...
                      <div key={p.name} className="flex items-center gap-2.5">
                          <div className="w-9 h-9 rounded-md bg-secondary shrink-0 overflow-hidden">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5">
                              ...
                              {p.badge &&
                            <span className={`text-[8px] font-body font-semibold px-1 py-0.5 rounded ${platformColors[p.platform]?.bg} ${platformColors[p.platform]?.text}`}>
                                  {p.badge}
                                </span>
                            }
                            </div>
                          ...
                          <div className="text-right shrink-0">
                            <span className="text-[12px] font-body font-bold text-accent block">{p.price}</span>
                            {p.oldPrice &&
                          <span className="text-[10px] font-body text-muted-foreground line-through">{p.oldPrice}</span>
                          }
                          </div>
                    ...
                    <div className="flex items-center justify-between mt-3 pt-3 border-t border-border">
                      ...
                      <Button variant="accent" size="sm" className="gap-1 text-[10px] rounded-full px-4 h-7">
                        Mua cả outfit 🛒
                      </Button>
                    </div>
                  </div>
